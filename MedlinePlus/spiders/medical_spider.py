...
class MedlinePlusspiderSpider(scrapy.Spider):
    ...
    # start_urls = ['https://medlineplus.gov/ency/article/001662.htm', ]
    ...
    def parse2(self,response):
        ...
        if test == "Causes":
            ...
            for texts in response.xpath(".//section"):
                ...
                text = p.sub(' ', text)
                p = re.compile(r'<p\b[^>]*>',re.S)  # 替换p为text
                ...
                if h2_key == "Symptoms":
                    item['Symptoms'] = text
                if h2_key == "Exams and Tests":
                    item['ExamsandTests'] = text
                if h2_key == "Treatment":
                    item['Treatment'] = text
                if h2_key == "Outlook (Prognosis)":
                    item['Prognosis'] = text
                if h2_key == "Possible Complications":
                    item['PossibleComplications'] = text
                if h2_key == "When to Contact a Medical Professional":
                    item['WhentoContactaMedicalProfessional'] = text
                if h2_key == "Prevention":
                    item['Prevention'] = text
            ...
            # 云服务器
            with open("E:/MedlinePlus/data/html/medical/" + name, "wb") as f:
            # 自己电脑
            #with open("D:/MedlinePlus/data/html/medical/" + "1.html","wb") as f:
